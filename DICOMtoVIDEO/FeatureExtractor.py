...
features_root_path='D:/Datasets/RADCURE/slowfastdeepfeatures/'
...
for videoID in range(2800,2899):
    video_fname=video_root_fname+video_name_list[videoID]
    features_path=features_root_path+'slowfast_'+video_name_list[videoID][:-3]+'txt'
    vr = decord.VideoReader(video_fname)
    
    if len(vr)>150:
        if len(vr)<=700:
            fast_frame_intervals=5
            slow_frame_intervals=25
        else:
            fast_frame_intervals=25
            slow_frame_intervals=100
        frame_id_list = list(range(0, len(vr), fast_frame_intervals))+list(range(0, len(vr), slow_frame_intervals))
