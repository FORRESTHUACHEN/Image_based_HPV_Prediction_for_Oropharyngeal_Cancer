...
root_source_path='D:/Datasets/RADCURE/RADCURE/'

video_store_path='D:/Datasets/RADCURE/VideosForFeatureExtraction/'


patient_ID=os.listdir(root_source_path)

for i in range(3300,3346):#len(patient_ID)):

    image_series_path=[]
    mask_path=[]
    
    patient_path=root_source_path+patient_ID[i]
    #for seconditem in range(len(os.listdir(patient_path))):
        #if 'Planning' in os.listdir(patient_path)[seconditem]:
    ...
    #image_array=image_array+1000
        image_array=image_array+125
    
        image_array[image_array<0]=0
        image_array[image_array>350]=350
        image_array=np.uint8(image_array*255/350)
        ...
        mask = rtstruct.get_roi_mask_by_name("GTVp")
